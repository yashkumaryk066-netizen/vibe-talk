...
const Login = ({ onSuccess }) => {
  ...
  return (
    <div className="screen h-screen w-full flex items-center justify-center relative overflow-hidden bg-cosmic">
      {/* 🌌 Premium 3D Background VFX */}
      <div className="stars"></div>
      <div className="absolute top-[-20%] left-[-20%] w-[60vw] h-[60vw] bg-purple-600/20 rounded-full blur-[120px] animate-float-slow"></div>
      <div className="absolute bottom-[-20%] right-[-20%] w-[60vw] h-[60vw] bg-cyan-600/20 rounded-full blur-[120px] animate-float-slow" style={{ animationDelay: '2s' }}></div>

      {/* Floating 3D Particles */}
      <div className="absolute top-1/4 left-1/4 w-2 h-2 bg-white rounded-full animate-pulse shadow-[0_0_10px_white]"></div>
      <div className="absolute bottom-1/3 right-1/3 w-3 h-3 bg-cyan-400 rounded-full animate-pulse shadow-[0_0_15px_cyan]" style={{ animationDuration: '3s' }}></div>

      {/* 💎 Glass Card Container */}
      <div className="z-10 w-full max-w-[400px] p-4 perspective-[1000px]">
        <div className="glass neon-border bg-black/40 backdrop-blur-2xl rounded-[30px] p-8 md:p-10 relative overflow-hidden group hover:rotate-y-[2deg] transition-transform duration-500 ease-out">

          {/* Shine Effect */}
          <div className="absolute top-0 left-[-150%] w-[200%] h-full bg-gradient-to-r from-transparent via-white/5 to-transparent skew-x-12 animate-shine pointer-events-none"></div>
          ...
          <div className="text-center mb-8 relative">
            <div className="relative inline-block">
              ...
              <div className="flex justify-center gap-1 h-6 items-end absolute -right-8 top-2 opacity-80">
                <div className="w-1 bg-cyan-400 animate-pulse h-3 rounded-full" style={{ animationDuration: '0.6s' }}></div>
                <div className="w-1 bg-blue-500 animate-pulse h-5 rounded-full" style={{ animationDuration: '0.4s' }}></div>
                <div className="w-1 bg-purple-500 animate-pulse h-3 rounded-full" style={{ animationDuration: '0.7s' }}></div>
                <div className="w-1 bg-pink-500 animate-pulse h-4 rounded-full" style={{ animationDuration: '0.5s' }}></div>
              </div>
            </div>
            <p className="text-white/60 text-sm font-medium tracking-widest uppercase mt-2">
              {isSignup ? "Initialize ID" : "Voice First Social"}
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6 relative z-10">
            ...
            {/* Username Input */}
            <div className="group/input neon-border rounded-xl bg-black/50 overflow-hidden">
              <div className="relative flex items-center">
                ...
                <input
                  type="text"
                  className="w-full bg-transparent text-white text-base p-4 pl-2 outline-none placeholder-white/30 font-medium"
            ...
            {isSignup && (
              <div className="group/input neon-border rounded-xl bg-black/50 overflow-hidden animate-in fade-in slide-in-from-top-2">
                <div className="relative flex items-center">
            ...
            <div className="group/input neon-border rounded-xl bg-black/50 overflow-hidden">
              <div className="relative flex items-center">
                <div className="pl-4 pr-2 text-pink-400">
                  <Flame size={20} /> {/* Used Flame as visual metaphor for 'Hot/Secret' or Lock */}
                </div>
                <input
                  type="password"
                  className="w-full bg-transparent text-white text-base p-4 pl-2 outline-none placeholder-white/30 font-medium"
            ...
            <button
              disabled={loading}
              className="w-full py-4 rounded-xl text-white font-bold text-lg bg-gradient-to-r from-cyan-500 via-blue-600 to-purple-600 hover:scale-[1.02] active:scale-[0.98] transition-all shadow-[0_0_20px_rgba(0,210,255,0.4)] relative overflow-hidden group-btn"
            >
              <span className="relative z-10 flex items-center justify-center gap-2">
                {loading ? "Connecting..." : (isSignup ? "Join the Vibe" : "Sign In")}
                {!loading && <ArrowLeft size={18} className="rotate-180" />}
              </span>
              <div className="absolute inset-0 bg-white/20 translate-y-full group-hover:translate-y-0 transition-transform duration-300"></div>
            </button>

            {/* Google Button (Visual Only for Aesthetics) */}
            {/* Google Button (Visual Only for Aesthetics) */}
            <button type="button" className="w-full py-3 rounded-xl bg-white text-black font-bold text-sm hover:bg-gray-100 transition-all flex items-center justify-center gap-3 shadow-[0_4px_15px_rgba(0,0,0,0.5)]">
              <svg width="20" height="20" viewBox="0 0 24 24" className="w-5 h-5 flex-shrink-0" xmlns="http://www.w3.org/2000/svg"><path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4" /><path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853" /><path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05" /><path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335" /></svg>
          ...
          <div className="mt-8 text-center">
            <p className="text-white/60 text-sm">
              ...
              <button
                onClick={toggleMode}
                className="text-cyan-400 font-bold hover:text-cyan-300 transition-all hover:underline"
              >
